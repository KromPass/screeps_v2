...
const rPC = require("./roles/powerCreep");
const c = require("./managers/city");
const m = require("./managers/markets");
const s = require("./operations/stats");
const rp = require("./managers/roomplan");
const er = require("./operations/error");
...
function loop() {
    ...
    profiler.wrap(function () {
        ...
        if (Game.time % 500 == 0) {
            ...
            claimRoom = c.chooseClosestRoom(myCities, (f.claim && f.claimRally) || f.claim);
            unclaimRoom = c.chooseClosestRoom(myCities, (f.unclaim && f.unclaimRally) || f.unclaim);
        ...
        for (let i = 0; i < myCities.length; i += 1) {
            try {
                ...
                const rclLimit = settings.bucket.colony - rcl * settings.bucket.rclMultiplier;
                if (rcl < 8 && Game.cpu.bucket < rclLimit && Game.gcl.level > 1) {
                    continue; // skip this city
                }
                c.updateCountsCity(city, localCreeps[city] || [], localRooms[city], claimRoom, unclaimRoom);
        ...
        if (Game.time % settings.scouting.assessTime == 0)
        ...
        if (Game.time % settings.flagCleanup)
            u.cleanFlags();
        data.makeVisuals();
        data.backupData();
        // disable emailing
