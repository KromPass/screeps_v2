...
const centralSpawn = {
    makeNextCreep: function (role, city, unhealthyStore = false, creepWantsBoosts = false, flag = null, budget = null) {
        ...
        const boostTier = creepWantsBoosts ? boosts_1.boosts.getBoostRank(role.actions, room) : 0;
        const recipe = types.getRecipe(role.name, energyToSpend, room, boostTier, flag);
        const spawns = room.find(FIND_MY_SPAWNS);
        if (!Memory.counter) {
            Memory.counter = 0;
        }
        const name = creepUtils_1.cU.generateCreepName(Memory.counter.toString(), role.name);
        ...
        if (boostTier > 0) {
            const boostsNeeded = boosts_1.boosts.getBoostsForRank(role.actions, boostTier);
