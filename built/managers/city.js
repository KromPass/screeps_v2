"use strict";
const centralSpawn = require("../buildings/spawn");
const creepUtils_1 = require("../lib/creepUtils");
const fact = require("../buildings/factory");
const labsLib = require("../buildings/labs");
const link = require("../buildings/link");
const motion = require("../lib/motion");
const pSpawn = require("../buildings/powerSpawn");
...
function runCity(city, creeps) {
    ...
    if (nextRole) {
        if (centralSpawn.makeNextCreep(nextRole, city, unhealthyStore, nextRoleInfo.boosted, nextRoleInfo.flag, nextRoleInfo.budget) && usedQueue) {
...
function updateQR(spawn, creeps) {
    if (Game.time % 100 == 5) {
        ...
        if (Memory.flags[flag]) {
            ...
            creepUtils_1.cU.scheduleIfNeeded("qrCode" /* QR_CODE_NAME */, creepsNeeded, false, spawn, creeps, flag);
        }
    }
}
...
function updateMilitary(city, memory, rooms, spawn, creeps) {
    ...
    const roles = ["harasser" /* HARASSER_NAME */, "powerMiner" /* POWER_MINER_NAME */, "depositMiner" /* DEPOSIT_MINER_NAME */];
...
function updateColonizers(city, claimRoom, unclaimRoom) {
    ...
    if (roomName == claimRoom) {
        ...
        if (Game.spawns[city].room.controller.level < 7) {
            creepUtils_1.cU.scheduleIfNeeded("spawnBuilder" /* SPAWN_BUILDER_NAME */, 4, true, Game.spawns[city], u.splitCreepsByCity()[city]);
        }
        else if (flag && Game.rooms[flag.roomName] && Game.rooms[flag.roomName].terminal) {
            creepUtils_1.cU.scheduleIfNeeded("spawnBuilder" /* SPAWN_BUILDER_NAME */, 4, true, Game.spawns[city], u.splitCreepsByCity()[city]);
        }
        else {
            creepUtils_1.cU.scheduleIfNeeded("spawnBuilder" /* SPAWN_BUILDER_NAME */, 2, true, Game.spawns[city], u.splitCreepsByCity()[city]);
        }
        if (flag && !Game.rooms[flag.roomName] || !Game.rooms[flag.roomName].controller.my) {
            creepUtils_1.cU.scheduleIfNeeded("claimer" /* CLAIMER_NAME */, 1, false, Game.spawns[city], u.splitCreepsByCity()[city]);
        }
    }
    if (roomName == unclaimRoom && Game.time % 1000 == 0) {
        sq.schedule(Game.spawns[city], "unclaimer" /* UNCLAIMER_NAME */);
    }
}
...
function updateDefender(spawn, rcl, creeps) {
    ...
    if (spawn.memory.towersActive) {
        ...
        if (hostiles > 3) {
            ...
            creepUtils_1.cU.scheduleIfNeeded("defender" /* DEFENDER_NAME */, Math.min(Math.floor(hostiles / 2), 4), true, spawn, creeps);
        }
    }
...
function requestSupport(spawn, quadsNeeded, rcl) {
    ...
    creepUtils_1.cU.scheduleIfNeeded("transporter" /* TRANSPORTER_NAME */, 2, false, reinforcingSpawn, creeps);
    creepUtils_1.cU.scheduleIfNeeded("quad" /* QUAD_NAME */, 4 * quadsNeeded, true, reinforcingSpawn, creeps, spawn.room.name, 400);
}
...
function updateMiner(creeps, rcl8, memory, spawn) {
    ...
    for (const sourceId in memory.sources) {
        creepUtils_1.cU.scheduleIfNeeded("remoteMiner" /* REMOTE_MINER_NAME */, 1, false, spawn, creeps, sourceId);
    }
}
    ...
    if (rcl > 5) {
        ...
        if (extractor) {
            ...
            if (room.terminal
                ...
                creepUtils_1.cU.scheduleIfNeeded("mineralMiner" /* MINERAL_MINER_NAME */, 1, false, spawn, creeps, room.name);
            }
        }
    }
}
function updateTransporter(extensions, memory, creeps, structures, spawn) {
    ...
    creepUtils_1.cU.scheduleIfNeeded("transporter" /* TRANSPORTER_NAME */, transportersNeeded, false, spawn, creeps, null, 200);
    memory["transporter" /* TRANSPORTER_NAME */] = 0; //TODO: remove
}
function updateUpgrader(city, controller, memory, rcl8, creeps, rcl) {
    ...
    if (rcl8) {
        ...
        if (controller.ticksToDowngrade < CONTROLLER_DOWNGRADE[rcl] / 2
            ...
            creepUtils_1.cU.scheduleIfNeeded("upgrader" /* UPGRADER_NAME */, 1, true, Game.spawns[city], creeps);
        }
    }
    else {
        const builders = _.filter(creeps, c => c.memory.role == "builder" /* BUILDER_NAME */).length + (sq.getCounts(Game.spawns[city])["builder" /* BUILDER_NAME */] || 0);
        const bank = roomU.getStorage(room);
        if (!bank)
            return;
        ...
        if (capacity < CONTAINER_CAPACITY) {
            if (!builders)
                creepUtils_1.cU.scheduleIfNeeded("upgrader" /* UPGRADER_NAME */, 1, false, Game.spawns[city], creeps);
            return;
        }
        ...
        creepUtils_1.cU.scheduleIfNeeded("upgrader" /* UPGRADER_NAME */, upgradersNeeded, rcl >= 6, Game.spawns[city], creeps);
        if (controller.ticksToDowngrade < CONTROLLER_DOWNGRADE[rcl] / 2) {
            creepUtils_1.cU.scheduleIfNeeded("upgrader" /* UPGRADER_NAME */, 1, rcl >= 6, Game.spawns[city], creeps);
        }
    }
}
function updateRepairer(spawn, memory, creeps) {
    ...
    creepUtils_1.cU.scheduleIfNeeded("repairer" /* REPAIRER_NAME */, repairersNeeded, false, spawn, creeps);
}
function updateBuilder(rcl, memory, spawn, creeps) {
    ...
    if (totalSites > 0) {
        if (storage.structureType == STRUCTURE_CONTAINER) {
            ...
            const upgraders = _.filter(creeps, c => c.memory.role == "upgrader" /* UPGRADER_NAME */).length;
            ...
            creepUtils_1.cU.scheduleIfNeeded("builder" /* BUILDER_NAME */, buildersNeeded, rcl >= 6, spawn, creeps);
        }
        else {
            creepUtils_1.cU.scheduleIfNeeded("builder" /* BUILDER_NAME */, settings.max.builders, rcl >= 6, spawn, creeps);
        }
    }
    if (rcl >= 4 && Game.cpu.bucket > settings.bucket.repair + settings.bucket.range * cityFraction(room.name) && spawn.room.storage && spawn.room.storage.store[RESOURCE_ENERGY] > settings.energy.repair) {
        ...
        if (walls.length) { //find lowest hits wall
            ...
            if (minHits < wallHeight || defenseMode) {
                creepUtils_1.cU.scheduleIfNeeded("builder" /* BUILDER_NAME */, 3, rcl >= 6, spawn, creeps);
                return;
            }
        }
        ...
        if (nukes.length) {
            ...
            for (const structure of nukeStructures) {
                ...
                if (!rampart) {
                    ...
                    sq.schedule(spawn, "builder" /* BUILDER_NAME */, rcl >= 7);
                    return;
                }
            }
        }
...
function updateRunner(creeps, spawn, extensions, memory, rcl, emergencyTime) {
    ...
    const miners = _.filter(creeps, creep => creep.memory.role == "remoteMiner" /* REMOTE_MINER_NAME */ && !creep.memory.link);
    ...
    const energyCarried = types.store(types.getRecipe("runner" /* RUNNER_NAME */, spawn.room.energyCapacityAvailable, spawn.room));
    ...
    creepUtils_1.cU.scheduleIfNeeded("runner" /* RUNNER_NAME */, runnersNeeded, false, spawn, creeps);
    memory["runner" /* RUNNER_NAME */] = 0; //TODO: remove
}
function updateFerry(spawn, rcl) {
    if (rcl >= 5) {
        creepUtils_1.cU.scheduleIfNeeded("ferry" /* FERRY_NAME */, 1, false, spawn, u.splitCreepsByCity()[spawn.name]);
    }
}
...
function updateHighwayCreep(flagName, spawn, creeps, role) {
    ...
    for (const flag of flagNames) {
        const boosted = role != "harasser" /* HARASSER_NAME */ || Memory.flags[flag].boosted && PServ;
        const numNeeded = role == "powerMiner" /* POWER_MINER_NAME */ && PServ ? 2 : 1;
        ...
        creepUtils_1.cU.scheduleIfNeeded(role, numNeeded, boosted, spawn, creeps, flag, distance);
    }
}
...
function runEarlyGame() {
    ...
    sq.schedule(spawn, "runner" /* RUNNER_NAME */, false, null, 100, -7);
    sq.schedule(spawn, "remoteMiner" /* REMOTE_MINER_NAME */, false, closestSource.id, 200, -6);
    sq.schedule(spawn, "runner" /* RUNNER_NAME */, false, null, 100, -5);
    sq.schedule(spawn, "upgrader" /* UPGRADER_NAME */, false, null, 200, -4);
    if (roomU.countMiningSpots(closestSource.pos) > 1) {
        sq.schedule(spawn, "remoteMiner" /* REMOTE_MINER_NAME */, false, closestSource.id, 300, -3);
    }
    sq.schedule(spawn, "remoteMiner" /* REMOTE_MINER_NAME */, false, otherSource.id, 300, -2);
    if (roomU.countMiningSpots(otherSource.pos) > 1) {
        sq.schedule(spawn, "remoteMiner" /* REMOTE_MINER_NAME */, false, otherSource.id, 200, -1);
    }
    Memory.gameState = 1;
}
function updateSpawnStress(spawn) {
    ...
    if (Game.time % 37 == 0) {
        ...
        const pastSpawnedTime = _.sum(localCreepMem, mem => Math.min(Math.max(mem.spawnTime - Math.max(windowStartTime - mem.spawnTick, 0), 0), Game.time - mem.spawnTick));
        const creepSpawnTime = pastSpawnedTime + queueTime;
        memory.spawnAvailability = spawns.length - (creepSpawnTime / CREEP_LIFE_TIME);
    }
}
function updateRemotes(city, myCreeps) {
    ...
    if (Game.time % 10 == 3) {
        const harasserRecipe = types.getRecipe("harasser" /* HARASSER_NAME */, Game.spawns[city].room.energyCapacityAvailable, Game.spawns[city].room);
        ...
        for (let i = 0; i < remotes.length; i++) {
            ...
            if (u.isSKRoom(remotes[i])) {
                ...
                if (spawn.room.controller.level < 7) {
                    creepUtils_1.cU.scheduleIfNeeded("quad" /* QUAD_NAME */, 1, false, spawn, myCreeps, remotes[i], 300);
                }
                else {
                    creepUtils_1.cU.scheduleIfNeeded("skGuard" /* SK_GUARD_NAME */, 1, false, spawn, myCreeps, remotes[i], 300);
                    if (Game.rooms[remotes[i]]) {
                        ...
                        if (mineral.mineralAmount > 0 && spawn.room.terminal && spawn.room.terminal.store[mineral.mineralType] < 6000) {
                            creepUtils_1.cU.scheduleIfNeeded("mineralMiner" /* MINERAL_MINER_NAME */, 1, false, spawn, myCreeps, remotes[i], 300);
                        }
                    }
                }
            }
            if (Game.rooms[remotes[i]]) {
                ...
                if (invaderCore && !u.isSKRoom(remotes[i])) {
                    ...
                    creepUtils_1.cU.scheduleIfNeeded("brick" /* BRICK_NAME */, bricksNeeded, false, spawn, myCreeps, remotes[i], 100);
                }
                ...
                if (spawn.room.energyCapacityAvailable >= reserverCost
                    ...
                    creepUtils_1.cU.scheduleIfNeeded("reserver" /* RESERVER_NAME */, reserversNeeded, false, spawn, myCreeps, remotes[i], 100);
                }
            }
            if (defcon == 2) {
                creepUtils_1.cU.scheduleIfNeeded("harasser" /* HARASSER_NAME */, 1, false, spawn, myCreeps, remotes[i], 300);
            }
            if (defcon == 3) {
                creepUtils_1.cU.scheduleIfNeeded("harasser" /* HARASSER_NAME */, 2, false, spawn, myCreeps, remotes[i], 300);
                creepUtils_1.cU.scheduleIfNeeded("quad" /* QUAD_NAME */, 4, false, spawn, myCreeps, remotes[i], 300);
            }
        }
    }
}
