...
const swcTrading_1 = require("./swcTrading");
const markets = {
    manageMarket: function (myCities) {
        if (PServ)
            swcTrading_1.simpleAllies.readAllySegment();
        if (Game.time % 10 != 0) {
            return;
        }
    ...
    distributeEnergy: function (myCities) {
        ...
        if (needEnergy.length) {
            ...
            if (PServ && Game.rooms[receiver].storage.store[RESOURCE_ENERGY] < 30000) {
                ...
                swcTrading_1.simpleAllies.requestResource(resourceRequest);
            }
        }
        if (!_.find(myCities, city => city.controller.level == 8)) {
            ...
            if (target && PServ && Game.rooms[target].storage && Game.rooms[target].storage.store[RESOURCE_ENERGY] < 600000) {
                const goalType = Game.rooms[target].controller.level == 6 ? 1 /* RCL7 */ : 2 /* RCL8 */;
                const funnelRequest = {
                    maxAmount: Game.rooms[target].controller.progressTotal - Game.rooms[target].controller.progress,
                    goalType: goalType,
                    roomName: target
                };
                swcTrading_1.simpleAllies.requestFunnel(funnelRequest);
            }
        }
    },
    ...
    buyAndSell: function (termCities) {
        ...
        if (PServ) {
            swcTrading_1.simpleAllies.initRun();
        }
        ...
        for (const city of termCities) {
            ...
            if (PServ) {
                swcTrading_1.simpleAllies.endRun();
            }
        ...
        for (const resource of resources) {
            if (container.store[resource] > threshold) {
                ...
                if (PServ) {
                    ...
                    for (const ally in swcTrading_1.simpleAllies.allyRequestMap) {
                        const requests = swcTrading_1.simpleAllies.allyRequestMap[ally];
