...
const p = {
    ...
    findBestRemote: function (city) {
        ...
        if (spawnFreeTime < settings.spawnFreeTime)
            return null;
        let distance = 1;
    ...
    calcSpawnTimeNeeded: function (roomName, spawn) {
        ...
        const minerBody = types.getRecipe("remoteMiner" /* REMOTE_MINER_NAME */, spawn.room.energyCapacityAvailable, spawn.room, 0, roomName);
        ...
        const harasserBody = types.getRecipe("harasser" /* HARASSER_NAME */, spawn.room.energyCapacityAvailable, spawn.room);
        const harasserCost = types.cost(harasserBody);
        const harasserSize = harasserBody.length;
        const quadBody = types.getRecipe("quad" /* QUAD_NAME */, spawn.room.energyCapacityAvailable, spawn.room);
        ...
        const sKGuardBody = types.getRecipe("skGuard" /* SK_GUARD_NAME */, spawn.room.energyCapacityAvailable, spawn.room);
    ...
    buildControllerLink: function (room, rcl) {
        ...
        const upgrader = _.find(creeps, c => c.memory.role == "upgrader" /* UPGRADER_NAME */);
        if (!upgrader)
            return;
        let location = null;
        let bestScore = 0;
        ...
        for (const source of sources) {
            ...
            for (let i = miner.pos.x - 1; i <= miner.pos.x + 1; i++) {
                ...
                for (let j = miner.pos.y - 1; j <= miner.pos.y + 1; j++) {
                    ...
                    for (const item of look) {
                        if (item.type == LOOK_STRUCTURES
                            || (item.type == LOOK_CREEPS && item[LOOK_CREEPS].memory.role == "remoteMiner" /* REMOTE_MINER_NAME */)
                            || (item.type == LOOK_TERRAIN && item[LOOK_TERRAIN] == "wall"))
                            go = false;
                    }
                    if (go) {
