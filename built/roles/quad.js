...
const rQ = {
    ...
    actions: ["damage" /* TOUGH */, "fatigue" /* MOVE */, "heal" /* HEAL */, "rangedAttack" /* RANGED_ATTACK */],
    /** @param {Creep} creep **/
    run: function (creep) {
        rQ.init(creep);
    ...
    isSafe: function (everythingByRoom, quad /*, destination*/) {
        ...
        for (let i = 0; i < rooms.length; i++) {
            ...
            const hostile = _.find(everythingByRoom[rooms[i]].hostiles, h => (creepUtils_1.cU.getCreepDamage(h, ATTACK) > 0 || creepUtils_1.cU.getCreepDamage(h, RANGED_ATTACK) > 0) &&
    ...
    rally: function (creep) {
        ...
        if (!flag || !rQ.isSafe(everythingByRoom, quad) || creep.room.name == flag.roomName || u.getRangeTo(quad[0].pos, flag) < 26) {
    ...
    yolo: function (quad) {
        for (let i = 0; i < quad.length; i++) {
            if (quad[i]) {
                ...
                quad[i].memory.role = "harasser" /* HARASSER_NAME */;
            }
        }
    },
            ...
            for (const member of roomName.creeps) {
                for (const attacker of melee) {
                    if (member.pos.isNearTo(attacker.pos) || (member.pos.inRangeTo(attacker.pos, 2) && !attacker.fatigue)) {
                        damage += creepUtils_1.cU.getCreepDamage(attacker, ATTACK);
                    }
                }
                for (const ranger of ranged) {
                    if (member.pos.inRangeTo(ranger.pos, 3) || (member.pos.inRangeTo(ranger.pos, 4) && !ranger.fatigue)) {
                        damage += creepUtils_1.cU.getCreepDamage(ranger, RANGED_ATTACK);
                    }
                }
            }
        }
        ...
        for (const roomName of Object.values(everythingByRoom)) {
            ...
            for (const creep of roomName.creeps) {
                if (_.find(hostiles, h => h.pos.isNearTo(creep.pos))
                    ...
                    creepUtils_1.cU.logDamage(creep, creep.pos, true);
                    continue;
                }
                ...
                if (structureTarget) {
                    ...
                    creepUtils_1.cU.logDamage(creep, structureTarget.pos);
                }
            }
        }
    },
                ...
                if (Game.rooms[roomName]) {
                    ...
                    for (const creep of Game.rooms[roomName].find(FIND_CREEPS)) {
                        ...
                        if (!quadNames.includes(creep.id)) {
                            ...
                            const attackThreat = creepUtils_1.cU.getCreepDamage(creep, ATTACK) > rQ.getDamageTolerance(quad);
