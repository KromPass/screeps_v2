...
const rR = {
    ...
    flipTarget: function (creep) {
        creep.memory.mode = creepUtils_1.cU.getNextLocation(creep.memory.mode, roomU.getTransferLocations(creep));
    },
    ...
    runController: function (creep) {
        ...
        if (creep.store.getFreeCapacity() == 0 && link.store.getFreeCapacity() > creep.store.getUsedCapacity()) {
            juicersNeeded += Math.floor(link.store.getFreeCapacity() / creep.store.getUsedCapacity());
        }
        if (juicersNeeded == 0) {
            creep.memory.juicer = false;
            return false;
    ...
    pickup: function (creep) {
        ...
        const goodLoads = creepUtils_1.cU.getGoodPickups(creep);
    ...
    getControllerRunnersNeeded: function (spawn) {
    ...
    updateControllerRunnersNeeded: function (spawn) {
        const upgraders = _.filter(u.splitCreepsByCity()[spawn.name], c => c.memory.role == "upgrader" /* UPGRADER_NAME */).length;
        const upgraderBody = types.getRecipe("upgrader" /* UPGRADER_NAME */, spawn.room.energyCapacityAvailable, spawn.room);
        const totalWorks = upgraders * upgraderBody.filter(part => part == WORK).length;
        const runnerBody = types.getRecipe("runner" /* RUNNER_NAME */, spawn.room.energyCapacityAvailable, spawn.room);
