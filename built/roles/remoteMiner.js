...
const rM = {
    name: "remoteMiner" /* REMOTE_MINER_NAME */,
    type: 5 /* miner */,
    run: function (creep) {
        if (creep.spawning) {
            return;
    ...
    checkRespawn: function (creep) {
        if (creep.ticksToLive == creep.memory.spawnBuffer + (creep.body.length * CREEP_SPAWN_TIME)) {
            ...
            creepUtils_1.cU.scheduleIfNeeded("remoteMiner" /* REMOTE_MINER_NAME */, 2, false, spawn, creeps, creep.memory.flag);
        }
    },
    ...
    maybeMove: function (creep, source) {
        if (creep.memory.moveStatus == "static" /* STATIC */) {
            if (!source) {
    ...
    isPositionBlockedMiner: function (roomPos) {
            ...
            if ((lookObject.type == LOOK_TERRAIN
                ...
                || (lookObject.type == LOOK_CREEPS
                    && (!lookObject[LOOK_CREEPS].my
                        || (lookObject[LOOK_CREEPS].memory.role == "remoteMiner" /* REMOTE_MINER_NAME */) && lookObject[LOOK_CREEPS].ticksToLive > 100))) {
                return true;
            }
        }
        return false;
