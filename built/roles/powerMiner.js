...
const rPM = {
    name: "powerMiner" /* POWER_MINER_NAME */,
    type: 17 /* powerMiner */,
    boosts: [RESOURCE_CATALYZED_GHODIUM_ALKALIDE, RESOURCE_CATALYZED_UTRIUM_ACID],
    actions: ["damage" /* TOUGH */, "attack" /* ATTACK */],
    run: function (creep) {
        creepUtils_1.cU.checkRoom(creep); //check if in hostile room
    ...
    summonRunners: function (creep, bank) {
        ...
        if (Game.time % 15 == 1 && bank.hits < creep.memory.bankInfo.summonHits) {
            ...
            creepUtils_1.cU.scheduleIfNeeded("runner" /* RUNNER_NAME */, creep.memory.bankInfo.runnersNeeded, false, localSpawn, localCreeps, creep.memory.flag);
        }
    },
