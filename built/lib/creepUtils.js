...
exports.cU = {
    setMoveStatus: function (creep) {
        if (!creep.memory.moveStatus)
            creep.memory.moveStatus = creep.getActiveBodyparts(MOVE) ? "mobile" /* MOBILE */ : "static" /* STATIC */;
    },
    ...
    getEnergy: function (creep) {
        const location = rU.getStorage(Game.spawns[creep.memory.city].room);
    ...
    getGoodPickups: function (creep) {
        ...
        return _.filter(pickups, pickup => exports.cU.isGoodPickup(creep, pickup, runnersBySource));
    },
    ...
    scheduleIfNeeded: function (role, count, boosted, spawn, currentCreeps, flag = null, tickOffset = 0) {
        const creepsInField = exports.cU.getCreepsByRole(currentCreeps, role);
        ...
        while (creepsNeeded > 0) {
            ...
            if (role == "powerMiner" /* POWER_MINER_NAME */) {
                sq.schedule(spawn, "medic" /* MEDIC_NAME */);
            }
            creepsNeeded--;
        }
    },
